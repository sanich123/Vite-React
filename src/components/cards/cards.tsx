...
export default function Cards({ formData }: { formData: FieldValues[] }) {
  return (
    <section className="cards">
      {formData.map(({ name, surname, city, zipcode, country, sexuality, gender, birthday, delivery, time, img, checkboxes, imgAlt }) => (
        <div key={`${name}${surname}`} className="cards__item item">
          <img src={img} alt={imgAlt} width="150px" height="150px" className="item__img" />
          ...
          <div className="item__email">
            <span className="item__text">{`Send me: ${checkboxes.length ? checkboxes.join(', ') : 'nothing'}`}</span>
          </div>
        </div>
      ))}
    </section>
